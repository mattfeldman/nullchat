processCommand = function (commandStub) {
    ...
    switch (args[0]) {
        case "create":
            ...
            break;
        case "moderator":
            if(!args[1]) throw new Meteor.Error("You must specify a valid user to make a moderator.");
            targetUser = Meteor.users.findOne({username:args[1]});
            if(!targetUser) throw new Meteor.Error("You must specify a valid user to make a moderator.");
            var room = args[2] ? Rooms.findOne({name: args[2]}) : commandStub.room;
            if(!room) throw new Meteor.Error("Invalid room specified.");
            if(Meteor.userId() !== room.ownerId) throw new Meteor.Error("You must be an owner to add a moderator.");
            if(_.contains(room.moderators,targetUser._id)) throw new Meteor.Error("User is already a moderator");
        ...
        case "topic":
            ...
            if(!_.contains(commandStub.room.moderators,Meteor.userId())) throw new Meteor.Error("You must be the owner to change topics.");
